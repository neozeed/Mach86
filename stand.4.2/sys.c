	...
	register struct iob *io;
{
	...
	if (isoldfs(&io->i_fs))
	{
#define	op	((struct odirect *)dp)
		daddr_t bn;
		int n,dc;

		dc = BSIZE;
		bn = (daddr_t)0;
		n = ip->i_size/sizeof(struct odirect);
		while(n--) {
			if (++dc >= BSIZE/sizeof(struct odirect)) {
				io->i_bn = fsbtodb(&io->i_fs, sbmap(io, bn++)) + io->i_boff;
				io->i_ma = io->i_buf;
				io->i_cc = BSIZE;
				devread(io);
				op = (struct odirect *)io->i_buf;
				dc = 0;
				if (bn == 1 && *((short *)(&op->od_name[2])))
					goto newdir;
			}

			if (match(s, op->od_name))
				return(op->od_ino);
			op = op + 1;
		}
		return(0);
...
badoff:
	...
	if (file->i_fs.fs_magic != FS_MAGIC
	    ...
	    ||
	    file->i_fs.fs_sbsize < sizeof(file->i_fs))
	{
