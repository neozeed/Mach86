...
****************************************************************************/
{
	...
	DBG (MKMsgAllocate) printf("Replymsg ");
	...
	(void) QueueMsg(replykmsgptr, 0, DontWait);
}
...
****************************************************************************/
{
	...
	if (PortArray[ServPort].Receiver == pKERNEL_PROCESS) {
		...
		result = vm_allocate(task_table[ipcp->task->proc - proc]->map, 
...
****************************************************************************/
{
	...
	if (PortArray[ServPort].Receiver == pKERNEL_PROCESS) {
		ipcp = PortArray[ServPort].Owner;
		result = vm_deallocate(task_table[ipcp->task->proc - proc]->map, 
