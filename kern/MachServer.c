...
ServPort = convert_port_to_task(MyMsgP->Head.LocalPort);
...
RepMsgP->Head.SimpleMsg = TRUE;
RepMsgP->Head.MsgSize =  sizeof (RepMessage);
ServPort = convert_port_to_thread(MyMsgP->Head.LocalPort);
...
RepMsgP->Head.SimpleMsg = TRUE;
RepMsgP->Head.MsgSize =  sizeof (RepMessage);
ServPort = convert_port_to_map(MyMsgP->Head.LocalPort);
...
/* Gather reply arguments */
 ...
 Boolean MachServer(InP, RepP)  caddr_t InP, RepP;
 { 
 typedef  struct { 
Msg	Head;
TypeType	RetCodeType;
